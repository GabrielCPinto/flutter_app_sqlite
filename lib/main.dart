...
class ItemRepository{
  ...
  Future <void> creat() async {
    final Future<Database> databas = openDatabase(
      join(await getDatabasesPath(), 'Item.db'),
      onCreate: (db, version) {
        return db.execute(
          "CREATE TABLE item(nome TEXT PRIMARY KEY,concluido BOOL)",
        );
      },
      version: 1,
    );
  ...
  Future < void > insertItem(Item item) async {
    // Geta referencetothedatabase.
    final Database db= await database;
    // InserttheDogintothecorrecttable. Alsospecifythe// `conflictAlgorithm`. In thiscase, ifthesamedogisinserted
    // multipletimes, it replacesthepreviousdata.
    await db.insert('item',item.toMap(),conflictAlgorithm: ConflictAlgorithm.replace,);
  }
  Future<void> updateItem(Item item) async{
    final db= await database;
    await db.update('item',item.toMap(),where: "nome = ?",whereArgs: [item.nome],);
  }
  Future<List<Item>> item() async{
    ...
    return List.generate(maps.length, (i) {
      return Item(
        concluido: maps[i]['concluido'],
        nome: maps[i]['nome'],
      );
    });
  }

  Future<void> deleteItem(String nome) async{
    final db= await database;
    await db.delete( 'item',where: "nome = ?",whereArgs: [nome],);
  }
